    ...
    $.fn.wslide = function(h){
        ...
        function gogogo(g){
            g.each(function(i){
				...
				a.css('overflow','hidden').css('position','relative').css('text-align','left').css('height',typey+'px').css('width','100%').css('margin','0').css('padding','0');
				a.find('ul').css('position','absolute').css('margin','0').css('padding','0').css('width','100%').css('height',Number(ligne*typey)+'px');
				b.css('display','block').css('overflow','hidden').css('float','left').css('height',litypey+'px').css('width','100%');
				b.each(function (i) {
					var offset = a.offset();
